...
> **不可跳过。** Debug/Coder 修完 bug 后，Leader 必须亲自验证修复是否正确。

修复完成后，Leader 逐条检查：

1. **读修复涉及的文件**（完整文件，不只看 diff）
2. **对照裁决清单逐条确认**：
   - 原始 bug 是否确实被修复
   - 修复是否引入了新问题
   - 修复方式是否合理（根因修复，不是表面补丁）
3. **判定结果**：
   - **全部满意** → 进入步骤 8
   - **部分不满意** → Leader 自己动手修正不满意的部分，然后进入步骤 8
   - **严重问题** → 重新分派 Debug 修复（回到步骤 7d），最多重试 1 次，仍不满意则 Leader 自己修

### 8. Leader 代码优化

修复审查通过后，Leader **亲自**阅读所有变更文件的**完整内容**（不只看 diff），逐项检查并动手修正。
...
**修复：** <无需修复 / 修复了 N 个 bug>
**修复审查：** <Leader 已验证全部修复 / Leader 自行修正 N 处>
...
3. Leader 裁决（7c）
4. 修复确认的 bug（7d）
5. **Leader 审查修复结果（7e）** — 不可跳过

### P8. Leader 代码优化

流程同步骤 8：Leader **亲自**阅读所有变更文件的**完整内容**，按步骤 8 的审查清单逐项检查并动手修正。

合并后 Leader 是唯一看到全貌的人，适合做跨模块优化、文件拆分等全局性改进。

### P9. 标记已完成的任务

流程同步骤 9：逐条对照代码确认，将已实现的任务在 tasks.md 中标记为 `[x]`。
