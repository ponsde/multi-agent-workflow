...
| **Coder** | GPT-5.3 Codex | nanoworker | 1-N | 读需求，自主实现代码+测试，交付 | 架构决策、跨模块判断 |
| **Debug** | GPT-5.3 Codex | nanoworker | 1-N | 审查代码、定位修复 bug、参与讨论 | 从零写功能、运行测试 |
| **Debug (Duel)** | GPT-5.3 Codex | nanoworker | 2-N | 对抗赛：独立找 bug + 环形互审 | 修复 bug、架构决策、运行测试 |
| **Tester** | GPT-5.3 Codex | nanoworker | 1 | 最终运行验证 | 写功能代码、直接修 bug |
...
| [nanoworker](examples/nanoworker-setup.md) | 轻量 Worker agent CLI，运行 GPT-5.3 Codex |
...
├── README.md                       # 本文
└── examples/
    ├── nanoworker-setup.md         # nanoworker 安装和配置指南
    ...
    ├── leader-agent-duel.md        # agent-duel 对抗赛流程示例
    ├── leader-agent-verify.md      # agent-verify 流程示例
    ├── coder-skill.md              # Coder 技能示例
    ├── debug-skill.md              # Debug 技能示例（普通模式）
    ├── debug-duel-skill.md         # Debug 对抗赛技能示例
