...
### 阶段零：agent-change-review（Leader + Coder 并行审查 change）

在 agent-apply 之前使用。Leader（全局视角）和 Coder（实现视角）并行审查 change 产出物，各自可直接修改，Leader 合并后汇总判断。

```
选择变更 → AI-CONTEXT 检查 → 读 change 产出物 → Leader 预评估
    │
    ▼
并行审查：
    ├─ Coder（在 worktree 中）→ 实现视角审查 + 修改
    └─ Leader（前台）→ 全局视角审查
    │
    ▼
Leader 合并 Coder 修改 → 综合报告 → 判断（通过/需修改/需讨论）
```
...
```
Leader 亲自验收（读所有变更文件的完整内容，6 维工程质量检查）
    │
    ├─ 缺功能 → Coder 补充 + Debug 审查（同阶段一）→ 重新验收
    │
    ├─ 有 bug → Debug 修复 → Leader↔Debug 讨论 → 重新验收
    │
    ├─ 结构/命名/维护性问题 → Leader 小修或回 agent-apply 重构
    │
...
普通 Debug 和对抗赛 Debug 是**不同的角色技能**（`debug-engineer` vs `debug-duel`），Leader 根据使用的流程（agent-apply vs agent-apply-duel）选择分派哪种。
...
.
├── README.md
├── BLUEPRINT.md                    # 多 agent 协作战略大纲
├── skills/                         # 技能文件
│   ├── README.md                   #   角色与部署说明
│   ├── leader/                     # Leader（编排者）
│   │   └── SKILL.md                #   核心编排：Agent Registry、消息模板、6 维验收
│   ├── opsx/                       # 编排流程（slash command 技能）
│   │   ├── agent-change-review.SKILL.md  #   Leader+Coder 并行审查 change
│   │   ├── agent-apply.SKILL.md          #   Coder 实现 + Debug 审查
│   │   ├── agent-apply-duel.SKILL.md     #   Coder 实现 + 对抗赛审查
│   │   └── agent-verify.SKILL.md         #   Leader 验收 + 路由（6 维检查 + 5 种路由）
│   ├── coder/
│   │   └── SKILL.md                # Coder 技能
│   ├── debug/
