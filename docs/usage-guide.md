...
# agent-apply 流程（opsx 编排技能）
mkdir -p ~/.claude/skills/openspec-agent-apply
cp skills/opsx/agent-apply.md ~/.claude/skills/openspec-agent-apply/SKILL.md

# agent-duel 流程（对抗赛审查）
mkdir -p ~/.claude/skills/openspec-agent-duel
cp skills/opsx/agent-duel.md ~/.claude/skills/openspec-agent-duel/SKILL.md

# agent-verify 流程
mkdir -p ~/.claude/skills/openspec-agent-verify
cp skills/opsx/agent-verify.md ~/.claude/skills/openspec-agent-verify/SKILL.md
```
...
cp rules/multi-agent.md ~/.claude/rules/common/multi-agent.md
```

> 详见 [../rules/multi-agent.md](../rules/multi-agent.md)。
...
默认 3 轮。修改 `skills/leader/orchestration.md` 和 `skills/opsx/agent-apply.md` 中的"3 轮"相关描述。
